      ...
      <Card className="mb-6">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3 min-w-0">
            <button onClick={() => navigate('/trademarks')} className="p-2 rounded-lg text-gray-400 hover:bg-gray-100 flex-shrink-0 mt-0.5">
              <ArrowLeft size={18} />
            </button>
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <h1 className="text-xl font-bold text-gray-900 break-words">{trademark.markName}</h1>
              ...
              <p className="text-sm text-gray-500 mt-1 break-words">{trademark.owner} · {trademark.jurisdiction}</p>
              {trademark.applicationNumber && (
                <p className="text-xs text-gray-400 mt-0.5">{trademark.applicationNumber}</p>
              )}
            </div>
          </div>
          <Button variant="secondary" size="sm" onClick={() => setShowEdit(true)} className="flex-shrink-0">
            <Edit2 size={14} /> Edit
          </Button>
        </div>
      </Card>
              ...
              {overdueDeadlines.length > 0 && (
                <div className="p-3 rounded-lg bg-red-50 border border-red-200">
                  ...
                  {overdueDeadlines.map(d => (
                    <div key={d.id} className="flex flex-wrap items-center justify-between gap-1 mt-1">
              ...
              {tmDeadlines.length === 0 && trademark.renewalDate && (
                <div className="p-3 rounded-lg bg-gray-50 border border-gray-100">
                  <div className="flex flex-wrap items-center justify-between gap-1">
                    ...
                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full flex-shrink-0 ${renewalUrgency.cls}`}>{renewalUrgency.text}</span>
                  </div>
