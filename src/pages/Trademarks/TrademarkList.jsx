  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 mb-4 flex flex-wrap gap-3 items-center">
        <SearchInput value={search} onChange={setSearch} placeholder="Search by name or jurisdiction..." className="w-64" />
        <select
          value={statusFilter}
                ...
                <tr key={tm.id} className="hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-0">
                  <td className="px-4 py-3">
                    <button
                      ...
                      className="font-semibold text-gray-900 hover:text-[#ffa600] transition-colors text-sm"
                    >
                      {tm.markName}
                    </button>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-1">
                      ...
                      {tm.niceClasses?.length > 3 && (
                        <span className="text-xs text-gray-400">+{tm.niceClasses.length - 3}</span>
                      )}
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => navigate(`/trademarks/${tm.id}`)} className="p-1.5 rounded-lg text-gray-400 hover:text-[#ffa600] hover:bg-orange-50 transition-colors" title="View">
                        <Eye size={15} />
                      </button>
                      <button onClick={() => setEditItem(tm)} className="p-1.5 rounded-lg text-gray-400 hover:text-blue-500 hover:bg-blue-50 transition-colors" title="Edit">
                        <Edit2 size={15} />
                      </button>
                      <button onClick={() => setDeleteItem(tm)} className="p-1.5 rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 transition-colors" title="Delete">
                        <Trash2 size={15} />
                      </button>
                    </div>
                  </td>
